...
def Login(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request, 'HumanFaces.html', {'data': 'Welcome ' + username})
    ...
    if request.method == 'POST':
        ...
        if password == confirm:
            ...
            if UserRegister.objects.filter(username=username).exists():
                return render(request, 'Register.html', {'data': 'User already exists'})

            UserRegister.objects.create(username=username, password=password)

            return render(request, 'Login.html', {'data': 'Registration Successful'})
